...
First Version of Original U-Net Decoder
"""
import torch
...
class FeatureFusion(nn.Module):
    def __init__(self, main_dim, aux_dim, out_dim, num_bands=6):
        super().__init__()
        self.num_bands = num_bands
        # Band-specific projections
        self.main_convs = nn.ModuleList([
            nn.Conv1d(main_dim, out_dim, kernel_size=1)
            for _ in range(num_bands)
        ])
        self.aux_convs = nn.ModuleList([
            nn.Conv1d(aux_dim, out_dim, kernel_size=1)
            for _ in range(num_bands)
        ])

        # Band-specific gating
    ...
    def forward(self, main_feat, aux_feat):
        B, L, C = main_feat.shape
        fused = torch.zeros(B, L, C, device=main_feat.device)

        for band_idx in range(self.num_bands):
            # Process entire feature vector for this band
            main_band = main_feat
            aux_band = aux_feat

            # Project using band-specific convolution
            main_proj = self.main_convs[band_idx](
                main_band.transpose(1, 2)
            ).transpose(1, 2)

            aux_proj = self.aux_convs[band_idx](
                aux_band.transpose(1, 2)
            ).transpose(1, 2)

            # Handle sequence length mismatch
            if aux_proj.size(1) < main_proj.size(1):
                aux_proj = F.pad(aux_proj, (0, 0, 0, main_proj.size(1) - aux_proj.size(1)))
            elif aux_proj.size(1) > main_proj.size(1):
                aux_proj = aux_proj[:, :main_proj.size(1), :]

            # Band-specific fusion
            combined = torch.cat([main_proj, aux_proj], dim=-1)
            gate = self.gates[band_idx](combined)
            fused_band = gate * main_proj + (1 - gate) * aux_proj

            # Extract only this band's features
            start_idx = band_idx * (C // self.num_bands)
            end_idx = (band_idx + 1) * (C // self.num_bands)
            fused[:, :, start_idx:end_idx] = fused_band[:, :, :(end_idx - start_idx)]
...
class SwinTransformer(nn.Module):
    def __init__(self, main_size=256, main_steps=12, main_spatch=4,
        ...
        self.feature_fusion = nn.ModuleList([
            FeatureFusion(main_dim=dims[i], aux_dim=dims[i], out_dim=dims[i], num_bands=out_chans)
            for i in range(len(depths))
        ])
