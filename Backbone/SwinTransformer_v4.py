...
class PatchMerging3D(nn.Module):
    ...
    def __init__(self,
                 ...
                 norm_only=False,
                 final_stage=False,  # No dimension expansion at final stage
                 ):
        super().__init__()
        self.dim = dim
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, resolution, depth, num_heads, window_size,
                 ...
                 norm_only=False, final_stage=False):
        super().__init__()
        self.dim = dim
...
class FeatureFusion(nn.Module):
    def __init__(self, out_dim):
        ...
        self.downsample_convs = nn.ModuleList([
            ...
            nn.Sequential(
                ...
                nn.Conv3d(out_dim, out_dim, kernel_size=(1, 1, 1))
            ),
        ])
    ...
    def forward(self, main_feat, main_resolution, aux_feat, aux_resolution, fusion_stage):
        ...
        gate_input = torch.cat([main_feat.mean(dim=1), aux_feat.mean(dim=1)], dim=-1)
        gate = self.gate_networks[fusion_stage](gate_input).unsqueeze(1)

        output = gate * main_feat + (1 - gate) * x
...
class SwinTransformer(nn.Module):
    def __init__(self, main_size=256, main_steps=12, main_spatch=4,
        ...
        x1_temporal_merges = [True, True, False, False]
        x1_norm_only = [True, True, False, False]
        ...
        for i in range(len(depths)):
            layer = BasicLayer(
                ...
                downsample=PatchMerging3D if i < len(depths) else None,
                temporal_merge=x_temporal_merges[i],
                final_stage=True if i == len(depths) - 1 else False
            )
            ...
            layer = BasicLayer(
                ...
                drop=drop_rate,
                attn_drop=attn_drop_rate,
                downsample=PatchMerging3D if i in [0, 1, 2, 3] else None,
                temporal_merge=x1_temporal_merges[i],
                norm_only=x1_norm_only[i],
                final_stage=True if i == len(depths) - 1 else False
            )
    ...
    def forward(self, x, x1, x2):
        ...
        for stage, (x_stage, x1_stage) in enumerate(zip(self.x_stages, self.x1_stages)):
            ...
            if stage == 0:
                self.encoder_features.append(x.view(B, x_tblk, x_hblk, x_wblk, -1))
            elif stage in [1, 2, 3]:
                # Convert to 3D for CNN Processing
                fusion_stage = stage - 1  # fusion after stage 2
                B, _, C = x1.shape
                x1_3d = x1.view(B, x1_tblk, x1_hblk, x1_wblk, -1).permute(0, 4, 1, 2, 3)
                cnn_feature = self.cnn_enhance[fusion_stage](x1_3d)
                cnn_feature = cnn_feature.permute(0, 2, 3, 4, 1)
                fused_feature = self.feature_fusion[fusion_stage](
                ...
                del x1_3d, cnn_feature
        x = self.decoder(x, original_shape, (x_tblk, x_hblk, x_wblk), self.encoder_features[::-1])
