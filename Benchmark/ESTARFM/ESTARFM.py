...
#   A fast version of ESTARFM code: improve the efficiency of original ESTARFM
#   through controlling the number of similar pixels

# ******************************************************************************************************
# *******************************Set parameters and read input data*************************************
def ESTARFM(config_path, landsat, modis, label_path, save_dir=None, save_name=None):
    ...
    num_similar_pixel = param['num_similar_pixel']   # set number of similar pixels, a smaller value is faster but accuracy may be lower
    ...
    ind_patch = np.zeros((n_nl * n_ns, 4), dtype=np.int64)
    ...
    for isub in range(0, n_nl * n_ns):
        ...
        data_F1 = FileName1[:, row1:row2 + 1, col1:col2 + 1]
        temp_data.append(data_F1)
    ...
    for isub in range(0, n_nl * n_ns):
        ...
        row1 = ind_patch[isub, 2]
        row2 = ind_patch[isub, 3]
        data_C1 = FileName2[:, row1:row2 + 1, col1:col2 + 1]
        temp_data.append(data_C1)
    ...
    for isub in range(0, n_nl * n_ns):
        ...
        row1 = ind_patch[isub, 2]
        row2 = ind_patch[isub, 3]
        data_F2 = FileName3[:, row1:row2 + 1, col1:col2 + 1]
        temp_data.append(data_F2)
    ...
    for isub in range(0, n_nl * n_ns):
        ...
        row1 = ind_patch[isub, 2]
        row2 = ind_patch[isub, 3]
        data_C2 = FileName4[:, row1:row2 + 1, col1:col2 + 1]
        temp_data.append(data_C2)
    ...
    for isub in range(0, n_nl * n_ns):
        ...
        row1 = ind_patch[isub, 2]
        row2 = ind_patch[isub, 3]
        data_C0 = FileName5[:, row1:row2 + 1, col1:col2 + 1]
        temp_data.append(data_C0)
    ...
    for isub in range(0, n_nl * n_ns):
        ...
        save_name = "pred.tif" if save_name is None else save_name
        temp_data.append(fine0)
    ...
    in_ds = gdal.Open(label_path)
    tempoutname1 = os.path.join(save_dir, "benchmark", "ESTARFM", save_name)
    suffix = os.path.splitext(tempoutname1)[1]
    if suffix == '.tif':
        driver = gdal.GetDriverByName("GTiff")
    elif suffix == "" or suffix == ".dat":
        driver = gdal.GetDriverByName("ENVI")
    dataset = driver.Create(tempoutname1, orig_ns, orig_nl, nb, gdal.GDT_Float32)

    data = temp_data[-1]
    for j in range(0, nb):
        dd = data[j, :, :]
        dataset.GetRasterBand(j + 1).WriteArray(dd, xOffset_list[0], yOffset_list[0])
